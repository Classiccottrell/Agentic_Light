...
description: "Ensures proper Python dependency management, avoiding global `pip install` and adhering to project-specific tooling. Use when running `pip install {package_name}`, adding or modifying Python packages/dependencies, initiating a new Python project or notebook (even just BigQuery cells), generating code with third-party `import` statements, or before running Python scripts via the terminal to confirm the correct virtual environment is active."
license: Apache-2.0
metadata:
  version: v1
  publisher: google
