...
{"name":"example-app","context_root":"app-context","default_scopes":["learning"],"roadmap":"ROADMAP.md"}
EOF
printf '%s\n' 'private workspace roadmap must not leak' > "$TMP/ROADMAP.md"
printf '%s\n' 'app learning: café, résumé, naïve, façade' > "$TMP/app-context/learning.md"

PACKET="$(bash "$ROOT/System_Config/context.sh" --root "$TMP" packet --profile example-app --query learning --max-bytes 160)"
case "$PACKET" in
  *'app learning'* ) : ;;
